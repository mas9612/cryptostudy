...
func TestCbcCtsMode(t *testing.T) {
	Nb = BlockSize128
	Nr = NumOfRounds128
	Nk = KeyLength128
	...
	for i, input := range inputs {
		cipherText := Cipher(input, keys[i], ModeCBC_CTS, ivs[i])
		if !bytes.Equal(cipherText, expected[i]) {
...
func TestInverseCbcCtsMode(t *testing.T) {
	Nb = BlockSize128
	Nr = NumOfRounds128
	Nk = KeyLength128
	...
	for i, input := range inputs {
		cipherText := InvCipher(input, keys[i], ModeCBC_CTS, ivs[i])
		if !bytes.Equal(cipherText, expected[i]) {
			t.Errorf("[TestCipher] case %d failed: cipherText != expected :\ncipher:\t\t%s\nexpected:\t%s", i, printableBytes(cipherText), printableBytes(expected[i]))
		}
	}
